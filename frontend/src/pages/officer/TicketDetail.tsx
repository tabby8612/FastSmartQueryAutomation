    ...
    <SidebarProvider style={{ "--sidebar-width": "calc(var(--spacing) * 72)", "--header-height": "calc(var(--spacing) * 16)" } as CSSProperties}>
      ...
      <SidebarInset>
        ...
        <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 p-4 md:p-8">
          <Link to="/officer/issues" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Back to issues</Link>
          {loading ? (
            ...
            <article className="overflow-hidden rounded-2xl border bg-card shadow-sm">
              ...
              <section className="space-y-4 border-b p-6" aria-labelledby="conversation-heading">
                ...
                {replies.filter(reply => reply.status === "sent").map(reply => (
                  <div key={reply.id} className="ml-4 space-y-2 rounded-xl border bg-primary/5 p-4 sm:ml-12">
                    <p className="text-sm font-semibold">Officer <span className="font-normal text-muted-foreground">· Sent</span></p>
              ...
              <section className="space-y-4 p-6" aria-labelledby="reply-heading" aria-busy={busy}>
                ...
                <Textarea id="reply-text" value={text} onChange={event => setText(event.target.value)} disabled={busy} placeholder="Dear Student,…" className="min-h-64 p-4 leading-7" />
